...
class NGramCreator:
    ...
    def __init__(self, dict):
        ...
        self.encoding_errors = dict.get('encoding_errors', 'strict')
        self.training_file_lines = sum(1 for line in open(self.training_file, encoding=self.encoding, errors=self.encoding_errors))
...
########################################################################################################################
    ...
    def _count(self, kind):
        if kind == "ip_list":
            with open(self.training_file, encoding=self.encoding, errors=self.encoding_errors) as input_file:
            ...
            with open(self.training_file, encoding=self.encoding, errors=self.encoding_errors) as input_file: # Open trainingfile
                for line in tqdm(input_file, desc=self.training_file, total=self.training_file_lines, disable=self.disable_progress, miniters=1000, unit="pw"):
                    line = line.rstrip('\r\n')
                    if len(line) != self.length: # Important to prevent generating "passwor", or "iloveyo", or "babygir"
                        continue
